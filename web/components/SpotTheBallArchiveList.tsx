...
import type { SpotTheBallSetArchiveItem } from "@/lib/api";

/**
 * Client island for the archive grid. Reads localStorage to surface
 * the player's score on each set they've already finished —
 * "✓ 287/500" overlay on completed rounds.
 *
 * Browser-only because localStorage isn't available during SSR; the
 * server page renders the cards without scores and this component
 * adds them after hydration.
 */
...
export function SpotTheBallArchiveList({
  ...
  sets: SpotTheBallSetArchiveItem[];
}) {
            ...
            <Link href={`/play/spot-the-ball/sets/${s.id}`} className="group block">
              <div className="relative aspect-video w-full overflow-hidden bg-ink-950">
                ...
                <img
                  ...
                  loading="lazy"
                />
                {summary && <ScoreBadge summary={summary} max={s.image_count * 100} />}
              </div>
              <div className="p-3">
